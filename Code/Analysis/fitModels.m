...
for ind = 1:length(data) % for all neurons
    ...
    os_temp(ind).Preference = data(ind).Preference;
    os_temp(ind).Satiety = data(ind).Satiety;
    os_temp(ind).LickVal = data(ind).LickVal;
    ...
    ms = helper_AllModeled(os_temp(ind), 'StartingPoints', nStart, 'RNG', RNG_val);
